...
class InnerModel:
    """Internal class that calculates the attributes of the inner model. Use the methods :meth:`~app.vendor.plspm.Plspm.inner_model`, :meth:`~app.vendor.plspm.Plspm.path_coefficients`, and :meth:`~app.vendor.plspm.Plspm.effects` defined on :class:`~.app.vendor.plspm.Plspm` to retrieve the inner model characteristics."""
    def __init__(self, path: pd.DataFrame, scores: pd.DataFrame):
        ...
        for dv in self.__endogenous:
            # pandas >=3.0 changed .loc[label,] (a 1-tuple key) indexing behavior;
            # .loc[label] (no trailing comma) is the equivalent, version-safe form.
