...
import pandas as pd, numpy as np, numpy.testing as npt, app.vendor.plspm.util as util, itertools as it, collections as c
from app.vendor.plspm.util import TopoSort
from app.vendor.plspm.mode import Mode
from app.vendor.plspm.scale import Scale
...
class Structure:
    ...
    Use this class to specify the relationships between constructs. It will generate a path matrix suitable for using in :class:`~.app.vendor.plspm.config.Config`.
    """
    ...
    def path(self):
        """Get a path matrix for use in :class:`~app.vendor.plspm.Config`.
        """
...
class MV:
    ...
    def __init__(self, name: str, scale: Scale = None):
        ...
        Args:
            ...
            scale: If using nonmetric data, the measurement type of the variable (otherwise ``None``). Takes a value from the enum :class:`~app.vendor.plspm.scale.Scale`. Note that if you specify a scale for one variable, you must specify a scale for all variables (or specify a default scale to use in the constructor).
        """
        self.__scale = scale
        self.__name = name
...
class Config:
    """Specify the model you want to calculate with :class:`~.app.vendor.plspm.Plspm`

    Create an instance of this class in order to specify the model you want to use :class:`~.app.vendor.plspm.Plspm` to calculate.
    """
    def __init__(self, path: pd.DataFrame, scaled: bool = True, default_scale: Scale = None):
        ...
        Args:
            ...
            default_scale: If your data is nonmetric, specify a default measurement type. Takes a value from the enum :class:`~app.vendor.plspm.scale.Scale`, or ``None`` (the default) if the data is metric and does not require scaling.
        """
        self.__modes = {}
        self.__mvs = {}
        self.__hoc = {}
    ...
    def add_lv(self, lv_name: str, mode: Mode, *mvs: MV):
        ...
        Args:
            ...
            mode: Whether the latent variable is reflective (mode A) or formative (mode B) with respect to its manifest variables. Takes a value from the enum :class:`~app.vendor.plspm.mode.Mode`
    ...
    def add_lv_with_columns_named(self, lv_name: str, mode: Mode, data: pd.DataFrame, col_name_starts_with: str,
        ...
        Args:
            ...
            default_scale: If the data is nonmetric, the measurement type of the manifest variables. Note that you can only use this method if all the manifest variables are the same type of measurement, otherwise use :meth:`add_lv`. Takes a value from the enum :class:`~app.vendor.plspm.scale.Scale`. ``None`` (the default) if the data is metric.
        """
