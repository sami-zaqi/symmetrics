...
import app.vendor.plspm.inner_summary as pis, app.vendor.plspm.config as c
import pandas as pd, numpy as np, app.vendor.plspm.weights as w, app.vendor.plspm.outer_model as om, app.vendor.plspm.inner_model as im
from app.vendor.plspm.scheme import Scheme
from app.vendor.plspm.unidimensionality import Unidimensionality
from app.vendor.plspm.bootstrap import Bootstrap
from app.vendor.plspm.estimator import Estimator
...
class Plspm:
    ...
    def path_coefficients(self) -> pd.DataFrame:
        ...
        Returns:
            a DataFrame of similar form to the Path matrix passed into :class:`app.vendor.plspm.config.Config`, with the relevant path coefficients in each cell
        """
